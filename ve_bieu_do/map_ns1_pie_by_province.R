...
gadm_vn <- geodata::gadm(country = "VNM", level = gadm_level, path = gadm_path, version = "4.0")$sf

# Xem tên cột tên tỉnh trong shapefile (thường là NAME_1)
print(names(gadm_vn))
# Thường tên tỉnh nằm ở cột NAME_1
if (!"NAME_1" %in% names(gadm_vn)) {
  stop("Không tìm thấy cột NAME_1 trong GADM - kiểm tra shapefile")
}

gadm_vn <- gadm_vn %>% mutate(NAME_1_lower = tolower(trimws(NAME_1)))

# --- KHỚP TÊN TỈNH GIỮA CSV VÀ SHAPE ---
# Thử khớp trực tiếp bằng lowercase
df_join <- df_wide %>%
  mutate(TINH_lower = tolower(trimws(TINH))) %>%
  left_join(gadm_vn %>% select(GID_1 = GID_1, NAME_1, NAME_1_lower, geometry), by = c("TINH_lower" = "NAME_1_lower"))
